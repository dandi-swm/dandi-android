...
class LoginUseCase @Inject constructor(
     ...
     *
     * 발급 토큰 저장은 백엔드 토큰 발급 로직 확정 후 별도 저장소(DataStore)와 함께 붙인다 —
     * 현재는 local.properties 의 마스터 토큰이 전 요청 헤더에 첨부된다.
     */
    suspend fun login(email: String, password: String): Result<AuthTokenVO> = try {
