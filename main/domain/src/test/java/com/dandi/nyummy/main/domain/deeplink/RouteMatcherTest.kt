...
class RouteMatcherTest {
    ...
    fun `static literal carries query params through`() {
        val route = matchRoute(
            segments = listOf("history"),
            query = mapOf("date" to "2026-07-19"),
            literalPaths = setOf("/history"),
            templates = emptyList(),
        )
        assertEquals(NavRoute("/history", mapOf("date" to "2026-07-19")), route)
    }
    ...
    fun `no match - returns null`() {
        val route = matchRoute(
            ...
            query = emptyMap(),
            literalPaths = setOf("/history", "/settings"),
            templates = listOf(articleTemplate),
        )
        assertNull(route)
    }
